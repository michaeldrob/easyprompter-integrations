...
export function getActionDefinitions(instance) {
    return {
        ...
        speed_up: {
            ...
            callback: (action) => {
                const step = action.options.step ?? DEFAULT_SPEED_STEP;
        ...
        set_speed: {
            ...
            callback: (action) => {
                const speed = Math.max(MIN_SPEED, Math.min(MAX_SPEED, action.options.speed));
        ...
        fast_forward_start: {
            ...
            callback: (action) => {
                const rate = action.options.rate ?? 1.0;
        ...
        shuttle_set: {
            ...
            callback: (action) => {
                const displacement = Math.max(-1.0, Math.min(1.0, action.options.displacement));
        ...
        jog_tick: {
            ...
            callback: (action) => {
                const direction = action.options.direction === "-1" ? -1 : 1;
                const magnitude = Math.max(1, Math.min(5, action.options.magnitude ?? 1));
        ...
        font_size_up: {
            ...
            callback: (action) => {
                const step = action.options.step ?? 2;
        ...
        line_height_up: {
            ...
            callback: (action) => {
                const step = action.options.step ?? 10;
        ...
        margin_up: {
            ...
            callback: (action) => {
                const step = action.options.step ?? 5;
        ...
        load_script: {
            ...
            callback: (action) => {
                const scriptId = action.options.scriptId?.trim();
                if (!scriptId)
                    return;
